  ...
  async processMemoryInBackground(userMessage, aiResponse, memberId, storage) {
    try {
      ...
      if (assessment.shouldRemember) {
        // Explicitly pass storageTarget from Sidecar to Storage
        const result = await storage.saveMemory({
          ...
          value: assessment.extractedInfo?.value,
          category: 'general', // Default category, can be refined by Sidecar later
          storageTarget: assessment.storageTarget || 'experience',
